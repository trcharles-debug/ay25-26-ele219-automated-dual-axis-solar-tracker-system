...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(arduino_motor_encoder_M, 0);
  ...
  arduino_motor_encoder_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(arduino_motor_encoder_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(arduino_motor_encoder_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(arduino_motor_encoder_M);
  ...
  /* Terminate model */
  arduino_motor_encoder_terminate();
