...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(arduino_encoder_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(arduino_encoder_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(arduino_encoder_M, "stopping the model");
}
