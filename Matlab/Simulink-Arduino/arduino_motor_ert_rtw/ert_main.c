...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (arduino_motor_M->Timing.clockTick0+
    arduino_motor_M->Timing.clockTickH0* 4294967296.0);
  arduino_motor_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(arduino_motor_M, 0);
  ...
  arduino_motor_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(arduino_motor_M->extModeInfo, &rtmGetTFinal
    (arduino_motor_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(arduino_motor_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(arduino_motor_M);
