...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    (arduino_encoder_inc_M->Timing.clockTick0+
     arduino_encoder_inc_M->Timing.clockTickH0* 4294967296.0);
  arduino_encoder_inc_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(arduino_encoder_inc_M, 0);
  ...
  arduino_encoder_inc_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(arduino_encoder_inc_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(arduino_encoder_inc_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(arduino_encoder_inc_M);
